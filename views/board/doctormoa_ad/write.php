...
<div class="content1">
	...
	?>
		...
		<ul class="com-cont-ul">
			...
			<?php } ?>
			<li class="cont-ul-tit">
                <span class="Sblod">광고제목</span>
                <input type="text" class="input px400" name="post_title" id="post_title" value="<?php echo set_value('post_title', element('post_title', element('post', $view))); ?>" placeholder="광고제목"/>
			...
			?>
				<li>
					<span  class="Sblod"><?php echo element('display_name', $value); ?></span>
			...
			<?php } ?>
			<li class="cont-ul-tit hidden">
                <span  class="Sblod">병원상세정보</span>
				<?php if ( ! element('use_dhtml', element('board', $view))) { ?>
					<div class="btn-group pull-right mb10 hidden">
            ...
            <?php if (element('use_post_tag', element('board', $view)) && element('can_tag_write', element('board', $view))) { ?>
                <li class="cont-ul-tit">
                    <span  class="Sblod">태그</span>
                    <input type="text" class="input" name="post_tag" id="post_tag" value="<?php echo set_value('post_tag', element('post_tag', element('post', $view))); ?>" />
			...
			?>
				<li class="cont-ul-tit">
					<span class="Sblod">병원 이미지 #<?php echo $i+1; ?></span>
                    <div class="fileBox">
                        <input type="text" class="fileName" id="fileName" name="fileName" readonly="readonly" placeholder="선택된 파일 없음">
                        <label for="file_<?php echo $i+1; ?>" class="btn_file">파일선택</label>
                        <input type="file" class="uploadBtn" name="<?php echo $file_column; ?>" id="file_<?php echo $i+1; ?>" />
                    ...
                    <?php if ($download_link) { ?>
                        ...
                        <label for="<?php echo $del_column; ?>" class="check-b">삭제 </label>
...
<script>
   ...
   $('input[name=address]#address').on("keyup",function () {
       new daum.Postcode({
           oncomplete: function(data) {
               var addr = data.address; // 최종 주소 변수
               // 주소 정보를 해당 필드에 넣는다.
