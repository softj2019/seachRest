...
<!--        </li>-->
        <li id="comment_<?php echo element('cmt_id', $result); ?>">
            <?php if (element('is_admin', $view)) { ?>
                <input type="checkbox" name="chk_comment_id[]" value="<?php echo element('cmt_id', $result); ?>" />
            <?php
            }
            ?>
                <p class="flexwrap"><?=element('cmt_depth', $result)?"A.":"Q."?> &nbsp;<?php echo element('content', $result); ?>
            ...
            ?>
            <?php
                    if (element('can_reply', $result) && $this->member->is_admin()) {
            ?>
                <button onClick="comment_box('<?php echo element('cmt_id', $result); ?>', 'c'); return false;" class="s-blue-btn qt-an-btn">답변</button>
            <?php
                }
            ?>
            <?php
                    if (element('can_update', $result)) { ?>
                <button onClick="comment_box('<?php echo element('cmt_id', $result); ?>', 'cu'); return false;" class="s-blue-btn qt-an-btn hidden">수정</button>
            <?php
                    }
            ?>
            <?php
                    if (element('can_delete', $result)) { ?>
                <button onClick="delete_comment('<?php echo element('cmt_id', $result); ?>', '<?php echo element('post_id', $result); ?>', '<?php echo element('page', $view); ?>');" class="s-blue-btn qt-an-btn hidden">삭제</button>
            <?php
                    }
            ?>
						<?php
