...
<header class="m-header">
    <div class="in-hd sub-hd">
        ...
        <div class="src-out">
            <p class="blue-txt">아이디 / 비밀번호 찾기</p>
...
<div class="in-box sign-box">
    <div class="content-box">
        <p class="blue-txt mb50">아이디 / 비밀번호 찾기</p>
        <?php
        echo validation_errors('<div class="alert alert-warning" role="alert">', '</div>');
        echo show_alert_message(element('message', $view), '<div class="alert alert-auto-close alert-dismissible alert-info"><button type="button" class="close alertclose" >&times;</button>', '</div>');
        $attributes = array('name' => 'findidpwform', 'id' => 'findidpwform','class'=>'sign-form');
        echo form_open(current_full_url(), $attributes);
        ?>
        <p>가입정보 입력</p>
        <input type="hidden" name="findtype" value="findidpw" />
        <h3>이메일 주소로 계정 찾기</h3>
        <span class="help-block">아이디/비밀번호는 가입시 등록한 메일 주소로 알려드립니다. 가입할 때 등록한 메일 주소를 입력하고 "찾기" 버튼을 클릭해주세요.</span>

        <input type="email" name="idpw_email" id="idpw_email" class="for-ph input-1" placeholder="Email Address" >
        <button class="blue-btn" type="submit">찾기</button>
...
<!--        <button type="button" class="blue-b-btn mt10">비밀번호 찾기</button>-->
        <?php
        echo form_close();
        ...
        ?>
            ...
            <span class="help-block">회원가입이나, 이메일주소 변경 후 인증 메일을 받지 못한 경우 다시 받을 수 있습니다.</span>
            <input type="email" name="verify_email" id="verify_email" class="for-ph input-1" placeholder="Email Address" />
            <button class="blue-btn" type="submit">인증메일 재발송</button>
