...
<div class="content">
   <div class="orderwrap">
        <h3 class="info-tit market-title">주문하기</h3>
        ...
        <table class="table markettable">
            <tr>
                <th>총 주문금액</th>
                <td class="pl30"><strong class="total_price2"><?php echo number_format($total_price_sum); ?>원</strong>
                    <?php
                    if ($this->cbconfig->item('use_deposit')) {
                        ?>
                        <br /><br />
                        보유<?php echo html_escape($this->cbconfig->item('deposit_name')); ?> : (<?php echo number_format((int) $this->member->item('total_deposit'));?>
        ...
        <div class="feedback-box mb20">
            <?php if ($this->cbconfig->item('use_payment_bank')) { ?>
                <input type="radio" name="pay_type" value="bank" id="pay_type_bank" />
                <label class="radio-inline" for="pay_type_bank" class="check-b"> 무통장입금</label>
            <?php } ?>
            <?php if ($this->cbconfig->item('use_payment_card')) { ?>
                <input type="radio" name="pay_type" value="card" id="pay_type_card" />
                <label class="radio-inline" for="pay_type_card" class="check-b">신용카드  </label>
            <?php } ?>
            <?php if ($this->cbconfig->item('use_payment_realtime')) { ?>
                <input type="radio" name="pay_type" value="realtime" id="pay_type_realtime" />
                <label class="radio-inline" for="pay_type_realtime" class="check-b">계좌이체</label>
            <?php } ?>
            <?php if ($this->cbconfig->item('use_payment_vbank')) { ?>
                <input type="radio" name="pay_type" value="vbank" id="pay_type_vbank" />
                <label class="radio-inline" for="pay_type_vbank" class="check-b">가상계좌 </label>
            <?php } ?>
            <?php if ($this->cbconfig->item('use_payment_phone')) { ?>
                <input type="radio" name="pay_type" value="phone" id="pay_type_phone" />
                <label class="radio-inline" for="pay_type_phone" class="check-b">휴대폰결제</label>
            <?php } ?>
        </div>
