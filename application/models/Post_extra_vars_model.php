...
class Post_extra_vars_model extends CB_Model
{
    ...
    public function get_all_sido()
    {

        $where = array(
            'pev_key' => 'address',
        );
        $select ='SUBSTRING_INDEX(pev_value, \' \', 1) as sido ,post_id';
        $this->db->select($select);
        $this->db->where($where);
        $this->db->group_by('sido');
        $qry = $this->db->get($this->_table);
        $result = $qry->result_array($qry);
        return $result;
    }
    public function get_all_address($sido)
    {
        ...
        $select ='SUBSTRING_INDEX(SUBSTRING_INDEX(pev_value, \' \', 2), \' \', -1) as sigun ,post_id';
        $this->db->select($select);
        $this->db->where($where);
        $this->db->where('SUBSTRING_INDEX(pev_value, \' \', 1) =', $sido);
        $this->db->group_by('sigun');
        $qry = $this->db->get($this->_table);
        $result = $qry->result_array($qry);
        return $result;
    }
