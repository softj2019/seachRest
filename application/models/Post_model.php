...
class Post_model extends CB_Model
{
	...
	public function get_search_list($limit = '', $offset = '', $where = '', $like = '', $board_id = 0, $orderby = '', $sfield = '', $skeyword = '', $sop = 'OR')
	{
		...
		$sop = (strtoupper($sop) === 'AND') ? 'AND' : 'OR';
		if (empty($sfield)) {
			$sfield = array('post_title', 'post_content','address');
		}
				...
				if (in_array($ssf, $this->search_field_equal)) {
					...
					foreach ($swordarray as $str) {
						...
						if ($sop === 'AND') {
                            ...
                            if($sfield=='address'){
                                $search_or_like[]=array('(select cb_post_extra_vars.pev_value from cb_post_extra_vars where cb_post_extra_vars.post_id = `cb_post`.post_id and cb_post_extra_vars.pev_key =\'address\')'=>$str);
                            }else{
                                $search_or_like[] = array($ssf => $str);
                            }
						}
